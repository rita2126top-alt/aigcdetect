...
def apply_lora(args, clip_model):
    list_lora_layers = []
    if args.lora_encoder == 'text' or args.lora_encoder == 'both':
        indices = INDEX_POSITIONS_TEXT[args.lora_position]
                ...
                for name, submodule in block.named_children():
                    if isinstance(submodule, nn.MultiheadAttention):
                        new_multi_head_lora = PlainMultiheadAttentionLoRA(
                            submodule, enable_lora=args.lora_params, r=args.lora_r, lora_alpha=args.lora_alpha, dropout_rate=args.lora_dropout_rate)
                        setattr(block, name, new_multi_head_lora)
                        list_lora_layers.append(new_multi_head_lora)

    if args.lora_encoder == 'vision' or args.lora_encoder == 'both':
        indices = INDEX_POSITIONS_VISION[args.backbone][args.lora_position]
