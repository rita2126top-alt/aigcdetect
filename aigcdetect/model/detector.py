...
def _ensure_upstream(ppmclip_root: str):
    root = str(Path(ppmclip_root).resolve())
    if root not in sys.path:
        sys.path.insert(0, root)
    try:
        import clip  # noqa: F401
        from loralib.utils import apply_lora, mark_only_lora_as_trainable  # noqa: F401
        from networks.DCT_score import DCTPatches  # noqa: F401
    except Exception as exc:
        raise RuntimeError(
            f"Cannot import upstream PPM_CLIP from {root}. Run: git submodule update --init --recursive"
        ) from exc
...
class ClassAnchoredDynamicPPMCLIP(nn.Module):
    ...
    def __init__(self, cfg: Dict):
        ...
        self.clip_model, _ = clip.load(model_path, device="cpu", jit=False)
    ...
    def patch_contrastive_loss(self, tokens_lnd, top_idx, bottom_idx):
        # tokens_lnd: [N+1,B,D]
        patches = tokens_lnd[1:].permute(1, 0, 2)
        dim = patches.size(-1)
    ...
    def encode_image_lora(self, image: torch.Tensor, training: bool):
        ...
        if training and not self.disable_dct_loss:
            top_idx, bottom_idx = self.dct(x_in)
        x = visual.conv1(x_in)
    ...
    def _encode_prompts(self, prompt_embeddings: torch.Tensor, eos_index: int, ctx_slice: slice):
        ...
        x = x + self.clip_model.positional_embedding.to(dtype=x.dtype, device=x.device)
        x = self.clip_model.transformer(x.permute(1, 0, 2)).permute(1, 0, 2)
        x = self.clip_model.ln_final(x)
        ctx = x[:, ctx_slice, :]
        eos = x[:, eos_index, :] @ self.clip_model.text_projection
    ...
    def _branch_probabilities(self, image_norm, patch_tokens, context):
        ...
        logits = self.temperature_image.exp() * torch.einsum("bd,bskcd->bskc", image_norm, proto)
    ...
    def forward(self, image: torch.Tensor, labels: torch.Tensor | None = None,
        ...
        if self.disable_dynamic_length:
            fixed_idx = len(self.length_candidates) - 1
        ...
        if self.disable_probabilistic_prompt:
            sample_num = 1
        flow_out = self.prompt_flow(global_feature, sample_num)
        if self.disable_probabilistic_prompt:
            flow_out["shared_bias"] = torch.zeros_like(flow_out["shared_bias"])
            flow_out["private_bias"] = torch.zeros_like(flow_out["private_bias"])
            flow_out["kl_loss"] = global_feature.new_zeros(())
            flow_out["rec_loss"] = global_feature.new_zeros(())
        ...
        probs_by_len = torch.stack(probs_by_len, dim=3)  # [B,S,K,R,2]
        logits_by_len = torch.stack(logits_by_len, dim=3)
        selector = length_hard[:, None, None, :, None]
        selected_probs = (probs_by_len * selector).sum(dim=3)  # [B,S,K,2]
        selected_logits = (logits_by_len * selector).sum(dim=3)
        ...
        if training:
            ...
            len_loss = self.length_router.expected_length_loss(length_probs, self.private_max)
            out["losses"] = {
                "cls": cls_loss,
